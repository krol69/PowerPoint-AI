...
│   ├── Src/
│   │   ├── App.fx.yaml         (App configuration + theme)
│   │   ├── scrHome.fx.yaml     (Home screen)
│   │   ├── scrProjectList.fx.yaml  (Project management)
│   │   ├── scrWizardStep1-4.fx.yaml  (4-step wizard)
│   │   ├── scrMySubmissions.fx.yaml  (User submissions)
│   │   ├── scrAdminDashboard.fx.yaml  (Admin panel)
│   │   ├── scrSubmissionReview.fx.yaml  (Review before submit)
│   │   ├── scrTickets.fx.yaml  (Support tickets)
│   │   └── scrTicketChat.fx.yaml  (Ticket messaging)
│   ├── Header.json             (App metadata)
│   ├── Properties.json         (App properties)
│   ├── Connections/            (Data connections)
│   ├── Entropy/                (Internal tracking)
│   └── pkgs/                   (Table definitions)
│
├── CanvasApp/                  ← Output folder for compiled .msapp
│   └── CrossDivProjectDB.msapp (Generated by repack script)
│
├── RepackToolkit/              ← Scripts to generate .msapp
│   ├── repack.ps1              (PowerShell version)
│   ├── repack.cmd              (Batch version)
│   └── README_REPACK.md        (Detailed repack instructions)
│
├── SeedData/                   ← CSV files for initial data
│   ├── Seed_SecurityFeatures.csv  (22 security features)
│   ├── Seed_AppConfig.csv         (5 app settings)
│   └── Seed_ChoiceValues.csv      (Reference for choice fields)
│
├── Docs/                       ← Complete documentation
│   ├── POST_IMPORT_CHECKLIST.md   (Step-by-step setup guide)
│   ├── DATAVERSE_SCHEMA.md        (Table schemas - 8 tables)
│   ├── TABLE_NAME_MAPPING_GUIDE.md (How to find table names)
│   ├── REPACK_PIPELINE.md         (Regenerate .msapp after changes)
│   └── THEME_DOCUMENTATION.md     (UI customization guide)
│
└── README.md                   ← This file
```
...
- **Protocols:** Multi-select (CAN, LIN, Ethernet, etc.)
  → If "Other" selected, must specify which protocols
- **Microcontroller:** Dropdown with "Other" option
  → If "Other" selected, must specify which microcontroller
...
5. **Import Seed Data** (5 min)
   - Import `SeedData/Seed_SecurityFeatures.csv` → Security Features table
   - Import `SeedData/Seed_AppConfig.csv` → App Configuration table

6. **Connect Data Sources** (10 min)
   - Open app in Power Apps Studio
   - Data panel → Add all 8 tables
   - See `Docs/TABLE_NAME_MAPPING_GUIDE.md` for help

7. **Configure Placeholders** (5 min)
   - Edit App.fx.yaml → OnStart
   - Update:
...
|-------------|----------|---------|-----------|
| `<<CHANGE_ADMIN_EMAIL>>` | App.fx.yaml → OnStart → gblConfig.AdminEmail | Admin notification email | **Yes** |
| `<<CHANGE_ADMIN_CC>>` | App.fx.yaml → OnStart → gblConfig.AdminEmailCC | CC recipients (comma-separated) | No (can be empty) |
| `<<CHANGE_SHAREPOINT_FOLDER_URL>>` | App.fx.yaml → OnStart → gblConfig.SharePointFolderURL | Document storage URL | **Yes** |
...
**Relationships:**
- Projects → Submissions (many-to-one)
- Project Security Details → Projects (many-to-one)
- Project Security Details → Security Features (many-to-one)
- Project Documents → Projects (many-to-one)
- Ticket Messages → Support Tickets (many-to-one)
...
Primary: #00BCD4 (cyan)      → CTAs, links, primary actions
Secondary: #7C4DFF (purple)  → Accents, secondary elements
Accent: #FF4081 (pink)       → Highlights, important badges
Success: #00E676 (green)     → Success states
Warning: #FFC107 (amber)     → Warnings
Error: #FF5252 (red)         → Errors, required field indicators
...
**A:** Go to make.powerapps.com → Tables → Click table → Properties panel (right side) shows the logical name. Full guide: `Docs/TABLE_NAME_MAPPING_GUIDE.md`

### Q: Can I change the 5-project limit?

**A:** Yes, edit `MaxProjects` in `gblConfig` (App.fx.yaml → OnStart). However, test thoroughly as UI layout is optimized for 5 projects.
...
See `Docs/POST_IMPORT_CHECKLIST.md` → Phase 5.
...
**A:**
1. Export your current app from Power Apps (Apps → Export)
...
3. Refresh data sources: Data panel → ⋮ → Refresh all

Full guide: `Docs/TABLE_NAME_MAPPING_GUIDE.md` → "If Tables Don't Appear"
...
- ✅ Enhanced repack scripts (PowerShell + Batch with validation)
- ✅ Comprehensive documentation (5 guides + README)
- ✅ Table name mapping guide for Dataverse environments
- ✅ Post-import checklist with 60-minute setup guide
- ✅ Verified all required features implemented

**Features:**
- ✅ 5-project limit per submission
- ✅ Clone project (fields + security details)
- ✅ Autosave timer (90 seconds) + save on navigation
- ✅ Resume to last screen/project
- ✅ Admin email notifications
- ✅ Protocols multi-select with "Other" validation
- ✅ Microcontroller dropdown with "Other" validation
- ✅ Security features with required specs
- ✅ SharePoint integration button
- ✅ Validation system blocking incomplete submissions
...
**Ready to get started?** → Run `RepackToolkit\repack.cmd` to generate your `.msapp` file!
